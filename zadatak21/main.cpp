...
*/
VEKTOR PAMETNIH POKAZIVACA -> IZMJENE !
#include <iostream>
#include <stdexcept>
#include <cstring>
#include <iomanip>
...
class Liga{
   ...
   int PronadjiTim(const char *ime) const{
    ...
    static bool funkcijakriterija(std::shared_ptr<Tim> tim1,std::shared_ptr<Tim> tim2){
...
//..KONSTRUKTOR
Liga::Liga():  timovi(std::make_shared<Tim>()){}
//...KONSTRUKTOR INICIJALIZACIJSKI
Liga::Liga(std::initializer_list<Tim> lista_timova): timovi(std::make_shared<Tim>()){
