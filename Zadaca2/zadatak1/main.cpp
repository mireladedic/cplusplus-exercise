...
//... OBRISI NIZ I POKAZIVAC POSTAVI NA NULL
        delete [] a[i];
        a[i] = nullptr;
...
int DodajOptuzbu(char **&a, int vel, std::string s){
    ...
    try{
       ...
       niz = new char [s.size() + 1];
        //... U NIZ PREPISE STRING
        strcpy(niz,s.c_str());
       //... POTRAZI MJESTO U NIZU POKAZIVACA
       for(int i (0); i < vel; i++){
            if(a[i] == nullptr) {
                a[i] = niz;
            }else{

                //...DINAMICKI ALOCIRA NOVI
            char **novi = new char *[vel + 1];
            novi = a;
            novi = niz;
            for(int j(0); j < vel; j++)
                //...OBRISI ONO NA STA POKAZUJU
                delete [] a[j];
        ...
        std::cout<<"Alokacija niza nije uspjela!\n";
        delete[] niz;
    }
